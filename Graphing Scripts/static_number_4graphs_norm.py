...
def build_plot(csv_path: Path, output_path: Path) -> None:
    ...
    baseline_variant = "DefaultAA"
    if baseline_variant not in variant_names:
        raise ValueError(f"{baseline_variant!r} is required in {csv_path} for normalization")
    ...
    for axis, name in zip(axes, plot_series):
        normalized_values_by_variant: dict[str, list[float]] = {}
        for variant in variant_names:
            normalized_values = []
            for benchmark in grouped_labels:
                baseline_index = grouped_indices[benchmark][baseline_variant]
                ...
                baseline_value = series_values[name][baseline_index]
                if baseline_value == 0:
                    normalized_values.append(0.0)
                else:
                    normalized_values.append(series_values[name][row_index] / baseline_value * 100)

            normalized_values_by_variant[variant] = normalized_values

        for offset, variant in zip(variant_offsets, variant_names):
            values = normalized_values_by_variant[variant]
            rects = axis.bar(x + offset, values, bar_width, label=variant, color=variant_colors.get(variant))
            axis.bar_label(rects, padding=3, fontsize=6, fmt="%.1f%%")
        ...
        metric_values = [
            normalized_values_by_variant[variant][benchmark_index]
            for benchmark_index in range(len(grouped_labels))
            for variant in variant_names
        ]
        axis.set_ylim(*y_axis_limits(metric_values))
        axis.axhline(100, color="#666666", linewidth=1, linestyle="--", alpha=0.8)
    ...
    fig.suptitle(f"{title} (DefaultAA = 100%)")
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", default="static_numbers_normalized.pdf", help="Path to the output PDF")
