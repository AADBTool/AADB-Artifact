...
def plot_performance(
    ...
    df = load_two_column_csv(csv_path)
    ...
    # ax.set_ylabel("Normalized Performance", fontsize=value_label_fs)
    ax.set_ylabel("Memory operations(%)", fontsize=value_label_fs)
    ...
    # ax.set_ylim(1, 24)
    ax.set_ylim(0.9, 2)
    # ax.set_yticks([ 0.7 + 0.05 * i for i in range(8)])
    # ax.set_yticks([ 1 + 2 * i for i in range(12)])
    ax.set_yticks([ 0.9 + 0.2 * i for i in range(6)])
