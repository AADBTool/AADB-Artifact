...
struct AAMDNodes {
  ...
  explicit AAMDNodes(MDNode *T, MDNode *TS, MDNode *S, MDNode *N, MDNode *M = nullptr)
      : TBAA(T), TBAAStruct(TS), Scope(S), NoAlias(N), MemID(M) {}

  bool operator==(const AAMDNodes &A) const {
    return TBAA == A.TBAA && TBAAStruct == A.TBAAStruct && Scope == A.Scope &&
           NoAlias == A.NoAlias && MemID == A.MemID;
  }

  bool operator!=(const AAMDNodes &A) const { return !(*this == A); }

  explicit operator bool() const {
    return TBAA || TBAAStruct || Scope || NoAlias || MemID;
  }
  ...
  /// The tag to get the metadata for the mem_id for load/store/call.
  MDNode *MemID = nullptr;
  ...
  AAMDNodes intersect(const AAMDNodes &Other) const {
    ...
    Result.MemID = Other.MemID == MemID ? MemID : nullptr;
    return Result;
  }
...
struct DenseMapInfo<AAMDNodes> {
  ...
  static unsigned getHashValue(const AAMDNodes &Val) {
    return DenseMapInfo<MDNode *>::getHashValue(Val.TBAA) ^
           ...
           DenseMapInfo<MDNode *>::getHashValue(Val.MemID);
  }
