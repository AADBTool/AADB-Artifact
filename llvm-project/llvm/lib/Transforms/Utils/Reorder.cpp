                    ...
                    for (auto it = defChain.rbegin(); it != defChain.rend(); ++it){
                      ...
                      if(dyn_cast<CallInst>(*it)){
                        if(CallInst *callInst = dyn_cast<CallInst>(*it)){
                          assert(EnableAcrossCallReordering && "Across call reordering should be enabled");
                          ...
                          if(GraphDefMRI == ModRefInfo::ModRef || GraphDefMRI == ModRefInfo::Ref || callInst->isVolatile()) { //Keep both mod and ref //callInst->mayHaveSideEffects() ||
                    ...
                    for (auto it = Int_ins.rbegin(); it != Int_ins.rend(); ++it){
                      ...
                      if (isa<StoreInst>(*it)){//&& (DA.depends(*it, startInst, true))){ 
                        ...
                        if(!EnableSpeculativeReordering){
                          ...
                          AliasResult GraphDefARChained_tail = AR;//(AR_tail == AliasResult::MayAlias) ? AR : AR_tail;
                          AliasResult GraphDefARChained_head = AR2;//(AR_head == AliasResult::MayAlias) ? AR2 : AR_head;
