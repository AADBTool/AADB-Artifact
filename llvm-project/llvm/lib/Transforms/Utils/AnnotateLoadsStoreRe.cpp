...
#include "llvm/Transforms/Utils/AnnotateLoadsStoresRe.h"
...
// PreservedAnalyses AnnotateLoadsStoresRePass::run(Module &M,
...
//                     auto *MD = MDNode::get(Ctx, {MDString::get(Ctx, "mem_id"),ConstantAsMetadata::get(ConstantInt::get(Type::getInt32Ty(Ctx), MemId_F))});
//                     I.setMetadata("mem_id", MD);
    ...
    for (Function &F : M) {
        for (BasicBlock &BB : F) {
            for (Instruction &I : BB) {
                if (isa<StoreInst>(&I)) {
                    LLVMContext &Ctx = I.getContext();
                    auto *MD = MDNode::get(Ctx, {MDString::get(Ctx, "mem_id"),ConstantAsMetadata::get(ConstantInt::get(Type::getInt32Ty(Ctx), MemId_F2))});
                    I.setMetadata("mem_id", MD);
