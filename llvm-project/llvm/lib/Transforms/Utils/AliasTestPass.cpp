...
AliasResult AliasTestPass::aliasInterp(const Value * V1, const Value * V2, AliasAnalysis &defaultAA) {
  ...
  auto paramName = valueToString(callParam);

  LLVM_DEBUG(dbgs() << "Iterating through callsite "<< *callsite 
                    << " ; Looking for " << paramName << " : " << *callParam << " in the call parameters :\n");

  bool usedByCall = (it != callsite->arg_end()) && it->get() == callParam;
  while(it != callsite->arg_end() && !usedByCall) {
    LLVM_DEBUG(dbgs() << "id : " << valueToString(it->get()) 
                      << " : " << *it->get() << "\n");
    usedByCall = (it->get() == callParam);
    if(!usedByCall) it++;
    ...
    for(auto * ptrN2 : *node2) {
      ...
      if(AAR == AliasResult::MustAlias) {
        ...
        if(auto * arg = dyn_cast<Argument>(ptrN1))
          infoPTN1 = "{func["+arg->getParent()->getName().str()+"]; arg["+valueToString(arg)+"]}";
...
void AliasTestPass::iterateOnFunction(Function &F, FunctionAnalysisManager &FAM,
  ...
  auto & SVFSAAR = FAM.getResult<SVFsAA>(F);
  auto & BasicAAR = FAM.getResult<BasicAA>(F);
  ...
  for (auto *Load : Sets.Loads) {
    ...
    for (auto *Store : Sets.Stores) {
      ...
      AliasResult ARSVF = SVFSAAR.alias(LoadLoc, StoreLoc, SimpleAAQI, nullptr);
      AliasResult ARBasic = BasicAAR.alias(LoadLoc, StoreLoc, SimpleAAQI, nullptr);
      errs() << "\n Alias test pass after query \n" << *Load << " <--> " << *Store << " ]\n";
      errs() << "SVFsAA result : " << ARSVF << "\n"
            << "Default AA pipeline result : " << ARFunc << "\n"
            << "GraphAA result : " << ARModule << "\n"
            << "GlobalsAA result : " << ARGlob << "\n"
            << "BasicAA result : " << ARBasic << "\n";
      if(betterAliasResult(ARSVF, ARFunc)) {
        errs() << "\n SVF performs better : \n" << *Load << " <--> " << *Store << " ]\n";
...
#ifdef COMPARE_RESULT
  errs() << "SVFsAA result that are better than default AA result :  " << betterResult << "\n"
...
void AliasTestPass::evaluate(Function &F, FunctionAnalysisManager &FAM, 
  ...
  std::vector<unsigned int> svfsAlias    (5,0);
  ...
  std::vector<unsigned int> svfsModRef    (5,0);

  AliasAnalysis &DefAAPipeline = FAM.getResult<AAManager>(F);
  SVFsAAResult &SVFsAAR = MAM.getResult<SVFsAA>(*(F.getParent()));
  ...
  for(auto * Load : Sets.Loads){
    ...
    for(auto * Call : Sets.Calls) {
      ...
      ModRefInfo SVFsMRI = SVFsAAR.getModRefInfo(Call, LoadLoc, SimpleAAQI);
      ...
      svfsModRef[(int)SVFsMRI]++; svfsModRef[4]++;
    ...
    for(auto * Store : Sets.Stores){
      ...
      AliasResult SVFsAR = SVFsAAR.alias(LoadLoc, StoreLoc, SimpleAAQI, nullptr);
      ...
      svfsAlias[(int)SVFsAR]++; svfsAlias[4]++;
  ...
  if(betterAlias) {
    ...
    PRINT_STAT("\033[32mSTANDALONE SVFS AA\033[0m", svfsAlias, false);
  ...
  if(betterModRef) {
    ...
    PRINT_STAT("\033[32mSTANDALONE SVFS AA\033[0m", svfsModRef, true);
