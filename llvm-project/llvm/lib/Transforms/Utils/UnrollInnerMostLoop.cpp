...
PreservedAnalyses UnrollInnermostLoopPass::run(Function &F, FunctionAnalysisManager &AM) {
    ...
    for (Loop *L : Loops) {
        ...
        // if (auto *CTC = SE.getConstantMaxBackedgeTakenCount(L)) {
        //     // llvm::outs() << "Loop trip count: ";
        //     // CTC->print(llvm::outs());
        //     assert(isa<SCEVConstant>(CTC) && "Trip count must be a constant");
        //     auto TripCount = cast<SCEVConstant>(CTC)->getAPInt().getLimitedValue();
        //     assert(TripCount > 3 && "Trip count must be at least 3"); //unroll loops above 3
        //     // llvm::outs() << "\n";
        // } else {
        //     llvm::outs() << "Loop trip count unknown\n";
        // }
    ...
    // Unroll the loop only if it contains 0 vector loads/stores and fewer than 100 instructions
    if ((((LoadInstructions != 0) || (StoreInstructions != 0)) && TotalInstructions < 100)) {
        // Ensure the loop is in LCSSA form before unrolling
