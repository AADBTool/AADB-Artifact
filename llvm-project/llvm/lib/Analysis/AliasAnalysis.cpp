...
INITIALIZE_PASS_DEPENDENCY(SVFsAAWrapperPass)
...
bool AAResultsWrapperPass::runOnFunction(Function &F) {
  ...
  if (auto *WrapperPass = getAnalysisIfAvailable<SCEVAAWrapperPass>())
    AAR->addAAResult(WrapperPass->getResult());
  if (auto *WrapperPass = getAnalysisIfAvailable<SVFsAAWrapperPass>()){
    assert(false && "SVF-based AA is enabled but SVFAAWrapperPass is not available!");
    AAR->addAAResult(WrapperPass->getResult());
  }
...
void AAResultsWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
  ...
  AU.addUsedIfAvailable<SVFsAAWrapperPass>();
