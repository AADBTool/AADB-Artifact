...
static cl::opt<bool> EnableSVFsAnalyses(
    "enable-svfs-analyses", cl::init(true), cl::Hidden,
    cl::desc("Enable SVFs inter-procedural analyses"));
...
AAManager PassBuilder::buildCapsUMUPipeline() {
  ...
  AA.registerFunctionAnalysis<SVFsAA>();
  // Add target-specific alias analyses.
  if (TM)
    TM->registerDefaultAliasAnalyses(AA);
...
AAManager PassBuilder::buildNoScevAAPipeline() {
  AAManager AA;
...
AAManager PassBuilder::buildDefaultChainedAAPipeline() {
  AAManager AA;
